...
import { toast } from "sonner"; // Using sonner for consistency with AddBlog
import { IApiError } from "@/types";
import { Loader2, LayoutGrid, UploadCloud, X, Plus } from "lucide-react";

// RTK Query Hooks (Ensure these are correctly exported from your API slice)
import {
...
const EditBlog = ({ className }: React.HTMLAttributes<HTMLDivElement>) => {
  const navigate = useNavigate();
  const { slug } = useParams<{ slug: string }>();

  // API Hooks
  const { data: blogData, isLoading: isFetching } = useGetBlogDetailsQuery(slug);
  ...
  useEffect(() => {
    if (blogData?.data) {
      ...
      setFormData({
        title: blog.title || "",
        slug: blog.slug || "",
        category: blog.category?.title || blog.category || "", // Handle if category is object or string
      ...
      if (blog.thumbnail || blog.thambnail) {
        setPreviewUrl(blog.thumbnail || blog.thambnail);
      }
    }
  }, [blogData]);
  ...
  useEffect(() => {
    ...
    // Only auto-generate if there is a title and we aren't currently fetching the initial load
    // This prevents the existing slug from being overwritten accidentally just because it loaded.
  ...
  }, [formData.title]); // Added purposely omitting isFetching to prevent loop
  ...
  const handleUpdateBlog = async (): Promise<void> => {
    ...
    // Reconstruct data exactly like AddBlog
    const payloadData = {
      ...
      deletedThumbnail: undefined as string | undefined,
    };
    ...
    if (formData.image) {
      ...
      // If a new image is uploaded, send the PREVIOUS image URL in the payload
      // (Checking both 'thumbnail' and 'thambnail' based on your previous data structure)
      const previousImageUrl = blogData?.data?.thumbnail || blogData?.data?.thambnail;
      if (previousImageUrl) {
        payloadData.deletedThumbnail = previousImageUrl;
      }
    }
    bodyFormData.append("data", JSON.stringify(payloadData));

    try {
      // Use your update mutation (adjust arguments to match your RTK endpoint)
      const res = await updateBlog({
        blogId: blogData?.data?._id,
        blogInfo: bodyFormData
      }).unwrap();
    ...
    <div className={cn("p-4 md:p-5", className)}>
      ...
      <div className="space-y-2 border-b border-gray-100 dark:border-zinc-800 pb-6 mb-8">
        ...
        <p className="text-base text-gray-500 dark:text-gray-400">
          Modify the content and settings of your existing article.
        </p>
      </div>
        ...
        <div className="w-full lg:w-2/3 flex flex-col gap-8">
          ...
          <Card className="border-gray-100 dark:border-zinc-800 shadow-sm rounded-2xl bg-white dark:bg-zinc-950">
            ...
            <CardContent className="pt-6">
              ...
              {/* FIX: Only render the editor once formData is populated by the API */}
              {formData.title ? (
                <TextEditor
                ...
                {previewUrl ? (
                  <div className="relative w-full h-full group">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <Button type="button" variant="destructive" size="icon" className="rounded-full w-10 h-10 shadow-lg" onClick={removeImage}>
